...
public class ConfigureJBossMojo extends AbstractJBossMojo
{
    ...
    private void checkJBossHome() throws MojoFailureException
    {
        ...
        {
            throw new MojoFailureException(jbossHomeDir.getAbsolutePath() + " does not appear to be a valid jboss home");
        }
        ...
        if(!serverDir.exists())
        {
            throw new MojoFailureException(serverName + " is not a valid server in " + serverParentDir.getAbsolutePath());
        }
    }
    ...
    {
        File baseDir = new File(jbossHome + File.separator + SERVER_DIR_NAME + File.separator + serverName + File.separator + dirName);
